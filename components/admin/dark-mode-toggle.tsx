...
export function DarkModeToggle() {
  ...
  // 썸 위치를 즉시 반영하기 위한 로컬 상태 — resolvedTheme 업데이트를 기다리지 않음
  ...
  // resolvedTheme 변경 시 checked 동기화
  useEffect(() => {
  ...
  const handleToggle = () => {
    // 1) 썸 위치를 즉시 업데이트 → React 리렌더 → CSS transition 동작
    const next = !checked;
    setChecked(next);

    // 2) 전역 색상 전환 애니메이션 활성화 후 테마 변경
  ...
  return (
    ...
    >
      {/* 썸(thumb) — checked 상태로 즉시 위치 반영 → CSS transition으로 슬라이딩 */}
      <span
        className={cn(
