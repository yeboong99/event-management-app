...
Phase 2 (참여자 관리 + 공지/댓글): 미착수 — TASK-022~031.

- Phase 2 PRD: 2026-03-22에 `docs/planning/Phase_2.md`로 생성. 13개 Task (DB 마이그레이션 3개, 타입 재생성, Zod 스키마 2개, Server Actions 2개, 페이지 4개, 라우팅 구조 1개).

**Why:** TaskMaster AI `parse-prd` 명령에 최적화된 형식으로 Phase별 PRD를 생성하여 체계적 태스크 관리를 수행하기 위함.
**How to apply:** 다음 Phase PRD 생성 시 Phase 2 완료 상태를 확인하고, 이전 Phase 산출물 섹션을 업데이트할 것. ROADMAP의 (host)/(participant) 경로를 실제 (app) 통합 구조로 변환하는 것 잊지 말 것.
