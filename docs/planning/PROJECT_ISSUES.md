...
| ID       | 카테고리 | 제목                                        | 상태      |
| -------- | -------- | ------------------------------------------- | --------- |
| FLOW-001 | UX/흐름  | 비공개 이벤트 참여 신청 흐름 단절           | ✅ 해결됨 |
| UI-001   | UI/UX    | 내 활동 페이지 이벤트 카드 비공개 뱃지 없음 | ✅ 해결됨 |
| UI-002   | UI/UX    | 정산 결과 배너 조사 오류 ("원를" → "원을")  | ✅ 해결됨 |

---

## UX / 흐름(Flow)

### [FLOW-001] 비공개 이벤트 참여 신청 흐름 단절 ✅ 해결됨 (2026-03-27)
...
### [UI-001] 내 활동 페이지 이벤트 카드에 비공개 이벤트 뱃지 없음 ✅ 해결됨
...
### [UI-002] 정산 결과 배너 조사 오류 ("원를" → "원을") ✅ 해결됨 (2026-03-27)

- **발견 시점**: 2026-03-27 (Phase 4 E2E 테스트)
- **영향 범위**: 이벤트 상세 페이지 → 정산 탭 → 내 정산 결과 배너
- **심각도**: 낮음 (UX 품질 문제)

#### 문제 설명

정산 결과 배너에서 한국어 조사가 잘못 적용됩니다.

**현재 출력 (오류):**

- 채권자 배너: "16,000원**를** 받을 수 있습니다"
- 채무자 배너: "2,000원**를** 보내야 합니다"

**올바른 출력:**

- "원"은 받침(`ㄴ`)이 있는 글자이므로 목적격 조사 "**을**"이 와야 함
- 채권자 배너: "16,000원**을** 받을 수 있습니다"
- 채무자 배너: "2,000원**을** 보내야 합니다"

#### 근본 원인

정산 결과 배너 컴포넌트(`components/shared/settlement-result.tsx`)에서 조사를 하드코딩하거나, 금액 숫자의 마지막 글자를 기준으로 조사를 자동 선택하는 로직이 없음.

#### 해결 방안

조사 선택 유틸 함수를 적용하거나, 단순히 "를" → "을"로 고정 수정. 금액 단위가 항상 "원"으로 끝나므로 "을" 고정도 가능.
